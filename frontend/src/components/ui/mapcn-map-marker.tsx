...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  // Sync center and zoom when props change (uncontrolled mode)
  const centerProp = props.center;
  const zoomProp = props.zoom;
  useEffect(() => {
    if (!mapInstance || isControlled) return;

    if (centerProp) {
      const currentCenter = mapInstance.getCenter();
      const currentCenterLngLat = [currentCenter.lng, currentCenter.lat];
      if (
        Math.abs(centerProp[0] - currentCenterLngLat[0]) > 0.0001 ||
        Math.abs(centerProp[1] - currentCenterLngLat[1]) > 0.0001
      ) {
        mapInstance.jumpTo({ center: centerProp });
      }
    }
  }, [mapInstance, isControlled, centerProp]);
  ...
  useEffect(() => {
    ...
    if (zoomProp !== undefined) {
      ...
      if (Math.abs(zoomProp - currentZoom) > 0.01) {
        mapInstance.jumpTo({ zoom: zoomProp });
      }
    }
  }, [mapInstance, isControlled, zoomProp]);
