...
async function run() {
    try {
        ...
        // Clear general/vegetables fallback and Amazon global assets/sprites from image cache
        console.log("Clearing bad cached images from ProductImageCache...");
        const delCache = await ProductImageCache.deleteMany({
            $or: [
                { imageUrl: /photo-1542838132-92c53300491e/i },
                { imageUrl: /sprite/i },
                { imageUrl: /\/images\/g\//i }
            ]
        });
        console.log(`Cleared ${delCache.deletedCount} bad cached entries.`);

        const shopType = "pharmacy";
        console.log("Clearing existing pharmacy products in Master Catalog...");
        ...
        console.log(`Processing ${productsToInsert.length} products...`);
        ...
        for (let idx = 0; idx < productsToInsert.length; idx += chunkSize) {
            ...
            const promises = chunk.map(async (item) => {
                ...
                if (!resolvedImage) {
                    resolvedImage = 'https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=500'; // Medical/medicine image fallback!
                }
            ...
            console.log(`Progress: ${count}/${productsToInsert.length} products seeded.`);
        }
